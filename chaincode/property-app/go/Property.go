...
// //////////////----changeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee---------------
package main
...
// OptimizationResult stores the output of the optimization algorithm.
// The JSON tags match what your server.js expects from 'data'.
type OptimizationResult struct {
	Objective           float64     `json:"objective"`
	P                   []float64   `json:"p"` // Productions, will be data.p
	Q                   [][]float64 `json:"q"` // Demands, will be data.q
	...
	ObjectivePlot []float64   `json:"objective_plot"` // Objective value per iteration
	LPlot         [][]float64 `json:"l_plot"`         // l_lagrange [l0, l1, l2] per iteration
	PPlot         [][]float64 `json:"p_plot"`         // p_prod [p0, p1, p2] per iteration
	SDPlot        [][]float64 `json:"sd_plot"`        // [TG, TD] per iteration
}
...
func (s *EnergyMarketContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	...
	initialResult := OptimizationResult{
		...
		L:                   make([]float64, numProducers),
		IterationsPerformed: 0,
		// Initialize empty plotting data
		ObjectivePlot: []float64{},
		LPlot:         [][]float64{},
		PPlot:         [][]float64{},
		SDPlot:        [][]float64{},
	}
...
func (s *EnergyMarketContract) Optimize(ctx contractapi.TransactionContextInterface, maxIterationsStr string) (*OptimizationResult, error) {
	...
	// Plotting data slices
	var objectivePlotData []float64
	var lPlotData [][]float64
	var pPlotData [][]float64
	var sdPlotData [][]float64 // For Total Generation (TG) and Total Demand (TD)
	...
	for k := 0; k < maxIterations; k++ {
		...
		cTotalCost := sumSlice(cCost)
		tgTotalGeneration := sumSlice(pProd) // For plotting
		...
		s1DemandPerProducerMarket = sumAxis0(qConsum)
		s2DemandPerConsumer = sumAxis1(qConsum)
		tdTotalDemand := sumSlice(s2DemandPerConsumer) // For plotting

		objective = zTotalUtility - cTotalCost

		// --- Collect data for plotting ---
		objectivePlotData = append(objectivePlotData, objective)
		lPlotData = append(lPlotData, copySlice(lLagrange)) // Important to copy
		pPlotData = append(pPlotData, copySlice(pProd))     // Important to copy
		sdPlotData = append(sdPlotData, []float64{tgTotalGeneration, tdTotalDemand})
	...
	finalResult := &OptimizationResult{
		...
		L:                   lLagrange,
		IterationsPerformed: iterationsPerformed,
		// Assign collected plotting data
		ObjectivePlot: objectivePlotData,
		LPlot:         lPlotData,
		PPlot:         pPlotData,
		SDPlot:        sdPlotData,
	}
